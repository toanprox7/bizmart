...
or:[{
  ...
  axios.post("/usersapi/search",infoSearch,{
    headers: {
...
handleSearch=_.debounce((text) => {
  ...
  if(this.props.match.params.textSearch != " "){
    ...
    axios.post('/usersapi/find',{skip:skipStart,sort:'createdAt DESC',limit:10},{
      headers: {
        'authorization':tokenAuthorization,
      }
    }).then(function (res) {
...
handleRequestSearch(){
  var self=this;
  if(this.props.match.params.textSearch == " "){
    axios.post('/usersapi/find',{sort:'createdAt DESC'},{
      headers: {
        'authorization':tokenAuthorization,
      }
    }).then(function (res) {
...
axios.post('/usersapi/updateStatus',info,{
  headers: {
    'authorization':tokenAuthorization,
  }
}).then(function (res) {
...
requestData= async (nextProps) =>{
  ...
  render() {
    ...
    <td>{this.state.data && this.state.data.role == 2?<b style={{color:"red"}}>Quản trị viên</b>:<b >Thành viên</b>}</td>
  <td>
