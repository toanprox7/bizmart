...
async handleChange(e){
  ...
  var self=this;
  axios.post('/usersapi/accessOldPassword',{id:parseInt(self.state.idUser),oldPassword:self.state.old_password}).then((res) => {
    // console.log(res)
if(res.data == "err"){
self.setState({
...
if(errors.length === 0){
  if(this.state.verify === false){
    ...
    axios.post("/usersapi/changePasswordAside",{id:parseInt(self.state.idUser),password:hashPass}).then(res => {
