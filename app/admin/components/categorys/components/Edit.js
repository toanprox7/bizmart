...
class EditForm extends Component {
  ...
  componentDidMount() {
    ...
    axios.post('/categoryapi/find', { id: this.props.match.params.id, sort: 'createdAt DESC' }).then(function (res) {
      self.setState({
        name: res.data.name,
      });
...
if(this.state.dataCategory && this.state.dataCategory.length >0){
  var dataFilterLabel = this.state.dataCategory.filter((item)=> item.id == data);
  // console.log(dataFilterLabel[0].name);
this.setState({
  selectedOption: { value: data, label:dataFilterLabel[0].name }
});
}
  ...
  handleSubmitEdit = (event, errors, values) => {
    var self = this;
    if (errors.length === 0) {
      axios.post('/categoryapi/update', { id: self.props.match.params.id, name: values.name, parentId: self.state.selectedOption.value }).then(function (res) {
        if (res.data) {
...
this.setState({
  displayModal:"none"
        ...
        <button type="button" onClick={
          () => {
            var self = this;
            axios.post('/categoryapi/destroy', { id: self.props.match.params.id }).then(function (res) {
              self.props.history.goBack();
            }).catch((err) => {
              console.log(err);
            })
