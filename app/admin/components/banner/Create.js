...
export default class CreateBanner extends Component {
    ...
    customProgressRenderer(progress, hasError, cancelHandler) {
      ...
      if (hasError || progress > -1 ) {
        ...
        if (progress === 100){
          message = (<span >Bạn đã tải lên ảnh thành công</span>);
        }
        return (
          <div>
    ...
    handleSubmit(e,err,value){
      ...
      if(errArray.length===0 && err.length === 0){
        ...
        axios.post("/banner/create",info,header).then(res=>{
          if(res.data === "success"){
      ...
      <div className="col-xs-12">
        <div className="box">
          <div className="box-header">
            <h3 className="box-title">Tạo banner mới</h3>
          </div>
          {/* /.box-header */}
          ...
          <AvField name="title" onChange={this.handleChange.bind(this)} placeholder="Vui lòng nhập tên banner" label="Tên banner" type="text" validate={{
            ...
            <AvField onChange={this.handleChange.bind(this)} name="link" placeholder="Vui lòng nhập đường dẫn banner" label="Đường dẫn banner" defaultValue={this.state.link?this.state.link:null} type="text" validate={{
          ...
          onLoad={ (e, request) => {
            let data = JSON.parse(request.response)[0].fd;
            let cutStr=data.slice(data.indexOf("/upload/")+8,data.length);
            this.setState({
              image:cutStr,
              errImage:null
            });
          ...
          <Button style={{marginTop:"15px"}} color="warning"><b>Tạo banner</b></Button>
          </AvForm>
          </div>
          </div>
          </div>
