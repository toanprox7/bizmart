...
class PaginationSearchProducts extends Component {
  ...
  componentDidMount() {
    ...
    let self =this;
    axios.post(`/productsapi/findProducts`,{title: {contains:this.props.textSearch},price:{ '>=': parseInt(start),'<=': parseInt(end) },limit:12,skip:skipStart}).then(function (res) {
      // console.log(res);
      self.setState({
      ...
      var self = this;
      axios.post(`/productsapi/findProducts`,{title: {contains:this.props.textSearch},limit:12,skip:skipStart})
        .then(function (res) {
          // console.log(res,"hii");
          self.setState({
            totalProducts:res.data.data
