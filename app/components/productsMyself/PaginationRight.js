...
class PaginationProductsMyselfRight extends Component {
  constructor(props) {
    super(props);
    this.state={
      activePage: 1
  ...
  componentDidMount() {
    ...
    let self =this;
    axios.post(`/productsapi/find`,{price:{ '>=': parseInt(start),'<=': parseInt(end) },status:'active',total_star:{ '>=': 4 }}).then(
    function (res) {
      // console.log(res);
      self.setState({
      ...
      var self = this;
      axios.post(`/productsapi/find`,{status:'active',total_star:{ '>=': 4 }})
        .then(function (res) {
          // console.log(res,"hii");
          self.setState({
            totalProducts:res.data,
  ...
  handleRedirect(numberPage){
    window.location.href=`/products-myself/${numberPage}/${this.props.price}`;
  }
  checkTotal(){
...
export default PaginationProductsMyselfRight;
