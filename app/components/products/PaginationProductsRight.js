...
class PaginationProductsRight extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let end = price.slice(getStt+1,price.length);
    let self = this;
    axios.post(`/productsapi/getAllProductsById`,{categoryId:nextProps.id,skip:skipStart,price:{ '>=': parseInt(start),'<=': parseInt(end) },limit:12},{
      headers: {
        'authorization':tokenAuthorization,
      }
    })
      ...
      let skipStart = (pageNumber - 1) * 12;
      let self =this;
      axios.post(`/productsapi/getAllProductsById`,{categoryId:nextProps.id,skip:skipStart,limit:12},{
        headers: {
          'authorization':tokenAuthorization,
        }
      })
