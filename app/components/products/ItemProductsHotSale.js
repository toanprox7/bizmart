...
class ItemProductsHotSale extends Component {
  constructor(props) {
    super(props);
    this.state={
...
const data = this.props.data;
 ...
 <div className="thumbnail">
                <a href={`/details/${ChangeToSlug(data.title)}-${data.id}-${data.categoryId.id}`}><img className="img-responsive" src={`/images/upload/small/${this.handleLinkImage()}`} alt="img_product" /></a>
                <div className="caption caption-height">
                  <a href={`/details/${ChangeToSlug(data.title)}-${data.id}-${data.categoryId.id}`}><h5>{data.title}</h5></a>
                  <p className="price">
...
export default ItemProductsHotSale;
